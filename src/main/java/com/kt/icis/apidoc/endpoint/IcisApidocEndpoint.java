...
import com.kt.icis.apidoc.config.IcisApidocsConfig;
...
@Slf4j
@RestController
public class IcisApidocEndpoint {

	private IcisApidocsConfig config;
	private List<ServiceTarget> serviceList = null;
	
	@Autowired
	public void setConfig(IcisApidocsConfig config) {
		this.config = config;
	}
	...
	public Map<String, Object> swaggerConfig(ServerHttpRequest serverHttpRequest) throws URISyntaxException {
		...
		for(int i=0; i < serviceList.size(); i++) {
			log.info("ICIS-APIDOC Service:[{}], Context-path:[{}]", serviceList.get(i).getName(), serviceList.get(i).getContextPath());
		...
		log.info("ICIS-APIDOC swagger urls:{}", swaggerConfig);
